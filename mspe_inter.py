"""
MSPE ablation: V-inter only.

Theory:
  Use only the inter-mode disagreement reduction term:

    ΔV_inter = V_inter_before - E_y[V_inter_after]

  For multi-output models, each output dimension is integrated independently
  with the same 1D quadrature machinery, and the resulting ΔV_inter values are
...
def compute_selection_score(rep_thetas, weights, cov_infos, X_cand, X_target, sigma2):
    """Compute V-inter only; V-intra is ablated out."""
    K = len(rep_thetas)
    Mc = X_cand.shape[0]
    D = n_out
    ...
    def _compute_inter_1d(mu_cand, s2_cand_1d, mu_test, cross_mats_1d):
        V_inter_before = 0.0
        for k in range(K):
            for l in range(k + 1, K):
                delta = mu_test[k] - mu_test[l]
                V_inter_before += weights[k] * weights[l] * np.dot(delta, delta)

        gains = [cross_mats_1d[k] / s2_cand_1d[k][None, :] for k in range(K)]

        pair_data = []
        for k in range(K):
            for l in range(k + 1, K):
                w_kl = weights[k] * weights[l]
                delta_kl = (mu_test[k] - mu_test[l])[:, None]
                a_kl = (delta_kl
                        - gains[k] * mu_cand[k][None, :]
                        + gains[l] * mu_cand[l][None, :])
                b_kl = gains[k] - gains[l]
                A = np.sum(a_kl ** 2, axis=0)
                B = 2.0 * np.sum(a_kl * b_kl, axis=0)
                C = np.sum(b_kl ** 2, axis=0)
                pair_data.append((w_kl, A, B, C))

        sig_cand = np.sqrt(s2_cand_1d)
        y_lo = float((mu_cand - 4.0 * sig_cand).min())
        y_hi = float((mu_cand + 4.0 * sig_cand).max())
        y_lo = max(y_lo, Y_MIN)
        y_hi = min(y_hi, Y_MAX)
        y_grid = np.linspace(y_lo, y_hi, N_QUAD)

        inv_sqrt_2pi = 1.0 / np.sqrt(2.0 * np.pi)
        phi = np.zeros((K, N_QUAD, Mc))
        for k in range(K):
            diff = y_grid[:, None] - mu_cand[k][None, :]
            phi[k] = inv_sqrt_2pi / sig_cand[k][None, :] * np.exp(
                -0.5 * diff ** 2 / s2_cand_1d[k][None, :])
        ...
        for k in range(K):
            for l in range(k + 1, K):
                ...
                pair_idx += 1
                ratio = phi[k] * phi[l] / p_mix
                poly = (A[None, :]
                        + B[None, :] * y_grid[:, None]
                        + C[None, :] * y_grid[:, None] ** 2)
                integral = np.trapezoid(ratio * poly, y_grid, axis=0)
                E_inter_after += w_kl * integral

        return np.maximum(V_inter_before - E_inter_after, 0.0)

    if D == 1:
        mu_cand = np.zeros((K, Mc))
        s2_cand_1d = np.zeros((K, Mc))
        mu_test = []
        cross_mats_1d = []
        ...
        for k in range(K):
            ...
            AJ_c = J_c_scaled @ H_inv_k
            a_k = np.sum(AJ_c * J_c_scaled, axis=1)

            pred_t, J_t = model_fn(rep_thetas[k], X_target)
            J_t_scaled = J_t * jac_scale_k[None, :]
            cross = J_t_scaled @ (H_inv_k @ J_c_scaled.T)

            mu_cand[k] = np.asarray(pred_c)
            s2_cand_1d[k] = sigma2 + a_k
            mu_test.append(np.asarray(pred_t))
            cross_mats_1d.append(cross)

        dv_inter = _compute_inter_1d(mu_cand, s2_cand_1d, np.array(mu_test), cross_mats_1d)
        return dv_intra, dv_inter

    # Multi-output: compute V-inter independently per output dimension and sum.
    for d in range(D):
        mu_cand = np.zeros((K, Mc))
        s2_cand_1d = np.zeros((K, Mc))
        mu_test = []
        ...
        for k in range(K):
            ...
            pred_c, J_c = model_fn(rep_thetas[k], X_cand)
            J_c_scaled = J_c * jac_scale_k[None, None, :]
            J_c_d = J_c_scaled[:, d, :]
            AJ_c = J_c_d @ H_inv_k
            a_k = np.sum(AJ_c * J_c_d, axis=1)

            pred_t, J_t = model_fn(rep_thetas[k], X_target)
            J_t_scaled = J_t * jac_scale_k[None, None, :]
            J_t_d = J_t_scaled[:, d, :]
            cross = J_t_d @ (H_inv_k @ J_c_d.T)

            mu_cand[k] = np.asarray(pred_c)[:, d]
            s2_cand_1d[k] = sigma2 + a_k
            mu_test.append(np.asarray(pred_t)[:, d])
            cross_mats_1d.append(cross)
...
if __name__ == '__main__':
    ...
    out_dir = os.path.join('output', DATASET, f"mspe_inter_{timestamp}")
    os.makedirs(out_dir, exist_ok=True)
    result_path = os.path.join(out_dir, 'result.json')

    print(f"=== mspe_inter === [{DATASET}]")
