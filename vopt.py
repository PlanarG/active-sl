"""
V-opt acquisition function (single best-fit mode).

Selects the next point by maximising the reduction in predictive variance
at the test points (V-optimal / D-optimal design), cost-penalised.
"""
import sys, os
...
# ── V-optimal acquisition ─────────────────────────────────────────────
def vopt_scores(theta, H_inv, jac_scale, X_cand, X_target, sigma2):
    """
    ΔV = tr(J_t Σ J_c^T (σ²I + J_c Σ J_c^T)^{-1} J_c Σ J_t^T)

    For D=1 this simplifies to Σ_i (j_t^T Σ j_c)² / (σ² + j_c^T Σ j_c).
    For D>1 we use the matrix version via einsum.
    """
    D  = n_out
    Mc = X_cand.shape[0]
    ...
    # Test Jacobians
    _, J_t      = model_fn(theta, X_target)
    J_t_s, _    = _scale_and_flatten_jac(J_t, jac_scale)   # (Mt,D,P) or (Mt,P)

    if D == 1:
        AJ_c  = J_c_s @ H_inv                               # (Mc, P)
        a_k   = np.sum(AJ_c * J_c_s, axis=1)               # (Mc,)  h(c)
        cross = J_t_s @ (H_inv @ J_c_s.T)                  # (Mt, Mc)
        dv    = (cross ** 2).sum(axis=0) / (sigma2 + a_k)  # (Mc,)
    else:
        # Multi-output V-optimal reduction:
        # dv_j = tr[(σ²I + J_c Σ J_c^T)^-1 (J_t Σ J_c^T)^T (J_t Σ J_c^T)]
        FJ_c  = np.einsum('mdp,pq->mdq', J_c_s, H_inv)     # (Mc, D, P)
        M_c   = np.matmul(FJ_c, J_c_s.transpose(0, 2, 1))  # (Mc, D, D)
        S_c   = sigma2 * np.eye(D)[None] + M_c             # (Mc, D, D)
        try:
            S_inv = np.linalg.inv(S_c)
        except np.linalg.LinAlgError:
            S_inv = np.zeros_like(S_c)
            for j in range(Mc):
                try:
                    S_inv[j] = np.linalg.inv(S_c[j])
                except np.linalg.LinAlgError:
                    pass

        J_t_flat = J_t_s.reshape(-1, n_params)             # (Mt*D, P)
        A_t = J_t_flat @ H_inv                             # (Mt*D, P)
        cross_all = np.einsum('ip,jdp->jid', A_t, J_c_s)   # (Mc, Mt*D, D)
        CTC_all = np.einsum('jid,jie->jde', cross_all, cross_all)  # (Mc, D, D)
        dv = np.einsum('mde,med->m', S_inv, CTC_all)
...
def run_one(seed, verbose=True):
    ...
    while cost_spent < budget:
        ...
        dv     = vopt_scores(best_theta, H_inv, jac_scale, X_cand, X_test, sigma2)

        cost_factor = (cost_train[affordable] / total_cost) ** COST_EXP
        scores      = dv / np.maximum(cost_factor, 1e-300)
        ...
        if verbose:
            ...
            print(f"  {iteration:3d}| idx={best_idx:3d} c={cost_train[best_idx]:.1e} "
                  f"dV={dv[best_local]:.4f} sig2={sigma2:.4e} "
...
if __name__ == '__main__':
    ...
    # all_tasks = [t for t in all_tasks if t.sl_id in ["sl_2"]]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_dir   = os.path.join('output', DATASET, f"vopt_0.4_{timestamp}")
    os.makedirs(out_dir, exist_ok=True)
    result_path = os.path.join(out_dir, 'result.json')

    print(f"=== vopt === [{DATASET}]")
