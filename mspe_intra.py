"""
MSPE ablation: V-intra only.

Theory:
  Use only the intra-mode predictive variance reduction term:

    ΔV_intra = Σ_k w_k · j_x^T Σ_k F_test Σ_k j_x / s_k²

  and discard the inter-mode disagreement term.
"""
import sys, os
...
def compute_selection_score(rep_thetas, weights, cov_infos, X_cand, X_target, sigma2):
    """Compute V-intra only; V-inter is ablated out."""
    K = len(rep_thetas)
    Mc = X_cand.shape[0]
    D = n_out
    ...
    if D > 1:
        # Multi-output: accumulate V-opt contribution over output dimensions.
        for k in range(K):
    ...
    # ── D == 1: same V-optimal intra term as the original MSPE script ──

    for k in range(K):
        H_inv_k, jac_scale_k = cov_infos[k]
        w_k = weights[k]

        _, J_c = model_fn(rep_thetas[k], X_cand)
        ...
        dv_k = (cross ** 2).sum(axis=0) / s2_k     # (Mc,)
        dv_intra += w_k * dv_k
...
if __name__ == '__main__':
    ...
    out_dir = os.path.join('output', DATASET, f"mspe_intra_{timestamp}")
    os.makedirs(out_dir, exist_ok=True)
    result_path = os.path.join(out_dir, 'result.json')

    print(f"=== mspe_intra === [{DATASET}]")
